...
materialName.addEventListener("change", function (e) {
  ...
  const materialDataArray = dataFromTableB.TableB.find(
    ({ MATERIAL }) => MATERIAL === materialNameValue
  );
...
const dataTableN = () => {
  ...
  for (const e of dataFromTableN.LightDuty) {
    screwDiam.innerHTML += `<option value="${e["SCREW DIA"]}">${
      e["SCREW DIA"] + '"'
    }</option>`;
  }
};
...
const dataTableV = () => {
  ...
  for (const e of dataFromTableV.TableV) {
    screwIncl.innerHTML += `<option value="${e["Inclination in °"]}">${
      e["Inclination in °"] + "°"
    }</option>`;
  }
};
...
const emptyFields = () => {
  ...
  const transmissionRelation = document.getElementById('transmissionRelation').value;
  if (massFlow === "" || lengthConveyor === "" || transmissionRelation === "") {
    return true;
  }
};
...
const loadFm = (conveyor) => {
  ...
  return data.find((e) => e["MATERIAL CLASS CODE"] === inputData)[
    "MATERIAL FACTOR"
  ];
};
...
const getScrewWeight = (conveyor) => {
  const flightsPerLength =
    conveyor.lengthConveyor /
    1000 /
    (conveyor.flightPitch * ((conveyor.screwDiameter * 25.4) / 1000));
  ...
  return (
    flightsPerLength * flightWeight +
    parseFloat(pipeWeight * (conveyor.lengthConveyor / 1000))
  );
};
...
const calculateDeflection = (conveyor) => {
  ...
  const deflection =
    ((screwWeight * Math.pow(conveyor.lengthConveyor / inToMm, 3)) /
      (76.7 * modulusOfElasticity * pipeInertia)) *
    inToMm;
...
const calculateEquivalentStress = (conveyor) => {
  ...
  const reducedStress = Math.pow(
    Math.pow(bendingStress, 2) + 3 * Math.pow(shearStress, 2),
    0.5
  );
...
calculate.addEventListener("click", function (e) {
  ...
  if (!emptyFields()) {
    ...
    screwConveyor.screwPipeSize =
      document.getElementById("screwPipeSize").value;
    screwConveyor.flightThickness =
      document.getElementById("flightThickness").value;
    ...
    screwConveyor.screwBoltGrade =
      document.getElementById("screwBoltGrade").value;
    screwConveyor.safetyFactor = document.getElementById("safetyFactor").value;
    screwConveyor.transmissionRelation = document.getElementById(
    ...
    screwConveyor.calculateConveyorSpeed(
      cf1,
      cf2,
      cf3,
      currentConveyorCapacity
    );
    ...
    document.getElementById("flowCapacity").value =
      screwConveyor.flowCapacity.toFixed(3);
    document.getElementById("realPower").value =
      screwConveyor.screwPower.toFixed(3);
    document.getElementById("closestPower").value =
      upperSizeMotor(screwConveyor);
    document.getElementById("transmissionRelationC").value =
      screwConveyor.transmissionRelation;
    document.getElementById("conveyorSpeed").value =
      screwConveyor.conveyorSpeed.toFixed(3);
    document.getElementById("reducerRatio").value =
      reducerData.reducerRatio.toFixed(3);
    document.getElementById("reducerSpeed").value =
      reducerData.reducerSpeed.toFixed(3);
    document.getElementById("screwBoltSF").value = screwBoltSF.toFixed(3);
    document.getElementById("screwDeflection").value =
      screwDeflection.toFixed(3);
    document.getElementById("reducerStress").value = reducerStress.toFixed(3);
    document.getElementById("screwPipeSF").value = screwPipeSF.toFixed(3);
